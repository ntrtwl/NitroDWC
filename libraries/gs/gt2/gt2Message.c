...
#include "gs/gt2/gt2Message.h"
#include "gs/gt2/gt2Buffer.h"
#include "gs/gt2/gt2Connection.h"
#include "gs/gt2/gt2Socket.h"
...
static GT2Bool gti2HandleReliableMessage(GT2Connection connection, GTI2MessageType type, GT2Byte * message, int len)
{
	unsigned short SN;
	unsigned short ESN;
	const int headerLength = GTI2_MAGIC_STRING_LEN + 1 + 2 + 2;
	GT2Bool overflow;

	// check if it's long enough
	if(len < (GTI2_MAGIC_STRING_LEN + 1 + 2 + 2))  // magic string + type + SN + ESN
	{
		if(!gti2ConnectionCommunicationError(connection))
			return GT2False;
	...
	// get the SN
	SN = gti2UShortFromBuffer(message, GTI2_MAGIC_STRING_LEN + 1);

	// get the ESN
	ESN = gti2UShortFromBuffer(message, GTI2_MAGIC_STRING_LEN + 3);
...
static GT2Bool gti2HandleUnreliableMessage(GT2Connection connection, GTI2MessageType type, GT2Byte * message, int len)
{
	...
	headerLength = (GTI2_MAGIC_STRING_LEN + 1);
...
static GT2Bool gti2HandleMessage(GT2Socket socket, GT2Byte * message, int len, unsigned int ip, unsigned short port)
{
	...
	magicString = ((len > GTI2_MAGIC_STRING_LEN) && (memcmp(message, GTI2_MAGIC_STRING, GTI2_MAGIC_STRING_LEN) == 0));
	...
	if(!connection)
	{
		...
		if(!magicString || (message[GTI2_MAGIC_STRING_LEN] != GTI2MsgClientChallenge))
		{
			// if this is a closed message, don't send one back (to avoid recursion)
			if(!magicString || (message[GTI2_MAGIC_STRING_LEN] != GTI2MsgClosed))
			{
	...
	if(magicString && ((len >= (GTI2_MAGIC_STRING_LEN * 2)) && (memcmp(message + GTI2_MAGIC_STRING_LEN, GTI2_MAGIC_STRING, GTI2_MAGIC_STRING_LEN) == 0)))
	{
	...
	type = (GTI2MessageType)message[GTI2_MAGIC_STRING_LEN];
...
GT2Bool gti2SendClientChallenge(GT2Connection connection, const char challenge[GTI2_CHALLENGE_LEN])
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgClientChallenge, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendServerChallenge(GT2Connection connection, const char response[GTI2_RESPONSE_LEN], const char challenge[GTI2_CHALLENGE_LEN])
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgServerChallenge, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendClientResponse(GT2Connection connection, const char response[GTI2_RESPONSE_LEN], const char * message, int len)
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgClientResponse, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendAccept(GT2Connection connection)
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgAccept, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendReject(GT2Connection connection, const GT2Byte * message, int len)
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgReject, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendClose(GT2Connection connection)
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgClose, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendKeepAlive(GT2Connection connection)
{
	...
	// begin the message
	if(!gti2BeginReliableMessage(connection, GTI2MsgKeepAlive, totalLen, &overflow))
		return GT2False;
...
GT2Bool gti2SendAppUnreliable(GT2Connection connection, const GT2Byte * message, int len)
{
	...
	if((len < GTI2_MAGIC_STRING_LEN) || 
		(memcmp(message, GTI2_MAGIC_STRING, GTI2_MAGIC_STRING_LEN) != 0))
	{
	...
	// copy the data at the starting position
	gti2BufferWriteData(&connection->outgoingBuffer, message, len);
...
{
	// always allocate magic string + type + ESN
	// part of the buffer may not be used but more efficience to be on stack
	char buffer[GTI2_MAGIC_STRING_LEN + 1 + 2];
	int pos = 0;
...
{
	// magic string + type + SNMin [+ SNMax]
	// part of the buffer may not be used but more efficience to be on stack
	char buffer[GTI2_MAGIC_STRING_LEN + 1 + 2 + 2];
	int pos = 0;
...
{
	// magic string + type + "time" + current time
	// part of the buffer may not be used but more efficience to be on stack
	char buffer[GTI2_MAGIC_STRING_LEN + 1 + 4 + sizeof(gsi_time)];
	int pos = 0;
	gsi_time now;
...
GT2Bool gti2SendClosedOnSocket(GT2Socket socket, unsigned int ip, unsigned short port)
{
	// magic string + type
	// part of the buffer may not be used but more efficience to be on stack
	char buffer[GTI2_MAGIC_STRING_LEN + 1]; 
	int pos = 0;
...
GT2Bool gti2ResendMessage(GT2Connection connection, GTI2OutgoingBufferMessage * message)
{
	...
	pos = (message->start + GTI2_MAGIC_STRING_LEN + 1 + 2);
	...
	type = (GTI2MessageType)connection->outgoingBuffer.buffer[message->start + GTI2_MAGIC_STRING_LEN];
