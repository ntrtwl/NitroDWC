...
#include "gs/gt2/gt2Filter.h"
#include "gs/gt2/gt2Utility.h"

#define GTI2_INVALID_IP_MASK     0xE0000000

static GT2Bool calling_closeall = GT2False;
...
{
	return gti2CreateSocket(socket, localAddress, outgoingBufferSize, incomingBufferSize, callback);
}
...
)
{
	...
	gti2Send(connection, message, len, reliable);
}
...
{
	if (calling_closeall == GT2True)
		return;

	calling_closeall = GT2True;

	TableMapSafe(socket->connections, gti2CloseAllConnectionsHardMap, NULL);

	calling_closeall = GT2False;
}
