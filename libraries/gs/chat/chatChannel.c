...
#define ASSERT_CHANNEL(line)     assertWithLine(channel != NULL, line); assertWithLine(channel[0] != '\0', line); assertWithLine(strlen(channel) < MAX_CHANNEL, line);
#define ASSERT_USER(user, line)  assertWithLine(user != NULL, line); assertWithLine(user[0] != '\0', line); assertWithLine(strlen(user) < MAX_NAME, line);
#define ASSERT_STR(str, line)    assertWithLine(str != NULL, line); assertWithLine(str[0] != '\0', line);
#define ASSERT_MODE(mode, line)  assertWithLine((mode >= 0) && (mode <= 3), line);
...
static int ciChannelTableHashFn(const void * elem, int numBuckets)
{
	...
	assertWithLine(elem != NULL, 180);
	assertWithLine(numBuckets > 0, 181);
...
static int ciChannelTableCompareFn(const void * elem1, const void * elem2)
{
	...
	assertWithLine(elem1 != NULL, 193);
	assertWithLine(elem2 != NULL, 194);

	str1 = ((ciChatChannel *)elem1)->name;
	str2 = ((ciChatChannel *)elem2)->name;
	assertWithLine(str1 != NULL, 198);
	assertWithLine(str2 != NULL, 199);
	ASSERT_STR(str1, 200);
	ASSERT_STR(str2, 201);
...
static int ciUserTableHashFn(const void * elem, int numBuckets)
{
	...
	assertWithLine(elem != NULL, 225);
	assertWithLine(numBuckets > 0, 226);
...
static int ciUserTableCompareFn(const void * elem1, const void * elem2)
{
	...
	assertWithLine(elem1 != NULL, 238);
	assertWithLine(elem2 != NULL, 239);
...
CHATBool ciIsEnteringChannel(CHAT chat, const char * channel)
{
	...
	for(i = 0 ; i < count ; i++)
	{
		...
		assertWithLine(chatChannel, 339);
...
void ciChannelEntered(CHAT chat, const char * channel, chatChannelCallbacks * callbacks)
{
	...
	ASSERT_CHANNEL(354);
	assertWithLine(callbacks != NULL, 355);
...
static void ciChannelListUsersMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 444);
	assertWithLine(clientData != NULL, 445);

	// Get the user.
	////////////////
	user = (ciChatUser *)elem;
	ASSERT_USER(user->name, 450);

	// Get the data.
	////////////////
	data = (ciChannelListUsersData *)clientData;
	assertWithLine(data->numUsers >= 0, 455);
#ifdef _DEBUG
	{
	int i;
	for(i = 0 ; i < data->numUsers ; i++)
	{
		ASSERT_USER(data->users[i], 461);
		ASSERT_MODE(data->modes[i], 462);
	}
	}
#endif
...
void ciChannelListUsers(CHAT chat, const char * channel, ciChannelListUsersCallback callback, void * param)
{
	...
	ASSERT_CHANNEL(498);
	assertWithLine(callback != NULL, 499);
...
void ciUserEnteredChannel(CHAT chat, const char * name, const char * channel, int mode, const char * user, const char * address)
{
	...
	ASSERT_USER(name, 722);
	ASSERT_STR(channel, 723);
	ASSERT_MODE(mode, 724);
	...
	// Check that we're in.
	///////////////////////
	assertWithLine(TableLookup(chatChannel->users, &chatUser) != NULL, 759);
}
...
void ciUserLeftChannel(CHAT chat, const char * user, const char * channel)
{
	...
	ASSERT_USER(user, 768);
	ASSERT_STR(channel, 769);
...
static void ciUserEnumChannelsMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 792);
	assertWithLine(clientData != NULL, 793);

	// Get the channel.
	///////////////////
	channel = (ciChatChannel *)elem;
	assertWithLine(channel->users != NULL, 798);

	// Get the user and callback.
	/////////////////////////////
	data = (ciUserEnumChannelsData *)clientData;
	assertWithLine(data->user != NULL, 803);
	assertWithLine(data->user->name[0] != '\0', 804);
	assertWithLine(data->callback != NULL, 805);
...
void ciUserEnumChannels(CHAT chat, const char * user, ciUserEnumChannelsCallback callback, void * param)
{
	...
	ASSERT_USER(user, 824);
	assertWithLine(callback != NULL, 825);
...
static void ciUserChangeNickMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 847);
	assertWithLine(clientData != NULL, 848);

	// Get the channel.
	///////////////////
	channel = (ciChatChannel *)elem;
	assertWithLine(channel->users != NULL, 853);

	// Get the data.
	////////////////
	data = (ciUserChangedNickData *)clientData;
	ASSERT_USER(data->newNick, 858);
	ASSERT_USER(data->oldNick, 859);
...
void ciUserChangedNick(CHAT chat, const char * oldNick, const char * newNick)
{
	...
	ASSERT_USER(oldNick, 904);
	ASSERT_USER(newNick, 905);
...
void ciUserChangedMode(CHAT chat, const char * user, const char * channel, int mode, CHATBool enabled)
{
	...
	ASSERT_USER(user, 925);
	ASSERT_STR(channel, 926);
	ASSERT_MODE(mode, 927);
...
static void ciEnumJoinedChannelsMap(void * elem, void * clientData)
{
	...
	ciChatChannel * channel;
	assertWithLine(elem != NULL, 1002);
	assertWithLine(clientData != NULL, 1003);
	...
	assertWithLine(data->callback != NULL, 1012);
...
static void ciSetUserBasicInfoMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 1050);
	assertWithLine(clientData != NULL, 1051);
	...
	///////////////////
	channel = (ciChatChannel *)elem;
	assertWithLine(channel->users != NULL, 1060);
...
static void ciGetUserBasicInfoMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 1105);
	assertWithLine(clientData != NULL, 1106);
	...
	///////////////////
	channel = (ciChatChannel *)elem;
	assertWithLine(channel->users != NULL, 1121);
...
static void ciClearAllUsersUsersMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 1206);
	assertWithLine(clientData != NULL, 1207);
...
static void ciClearAllUsersChannelMap(void * elem, void * clientData)
{
	...
	assertWithLine(elem != NULL, 1229);
	assertWithLine(clientData != NULL, 1230);

	// Get the channel.
	///////////////////
	channel = (ciChatChannel *)elem;
	assertWithLine(channel->users != NULL, 1235);
