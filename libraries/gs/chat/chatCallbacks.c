...
** DEFINES **
************/
#define ASSERT_DATA(data, line)   assertWithLine(data != NULL, line); assertWithLine(data->type >= 0, line); assertWithLine(data->type < CALLBACK_NUM, line); assertWithLine(data->callback != NULL, line); assertWithLine(data->callbackParams != NULL, line);  assertWithLine(data->ID >= 0, line);
...
#define COPY_STR_ARRAY(array, num, line)  assertWithLine(srcParams->num >= 0, line);\
									if(!srcParams->array)\
...
#define COPY_INT_ARRAY(array, num, line)  assertWithLine(srcParams->num >= 0, line);\
                                    if(srcParams->num > 0)\
									{\
										assertWithLine(srcParams->array != NULL, line);\
...
CHATBool ciAddCallback_(CHAT chat, int type, void * callback, void * callbackParams, void * param, int ID, const char * channel, size_t callbackParamsSize)
{
	...
	assertWithLine(type >= 0, 545);
	assertWithLine(type < CALLBACK_NUM, 546);
	assertWithLine(connection->callbackList != NULL, 547);
	assertWithLine(callback != NULL, 548);
	assertWithLine(callbackParams != NULL, 549);
	assertWithLine(callbackParamsSize > 0, 550);
	assertWithLine(ID >= 0, 551);
#ifdef _DEBUG
	if(channel != NULL)
		assert(channel[0] != '\0');
#endif
	...
	case CALLBACK_ENUM_CHANNELS_ALL:
	{
		...
		COPY_STR_ARRAY(channels, numChannels, 723);
		COPY_STR_ARRAY(topics, numChannels, 724);
		COPY_INT_ARRAY(numUsers, numChannels, 725);
		break;
	}
	...
	case CALLBACK_ENUM_USERS:
	{
		...
		COPY(channel);
		COPY_STR_ARRAY(users, numUsers, 769);
		COPY_INT_ARRAY(modes, numUsers, 770);
		break;
	}
	...
	case CALLBACK_GET_USER_INFO:
	{
		...
		COPY(name);
		COPY(address);
		COPY_STR_ARRAY(channels, numChannels, 782);
		break;
	}
	...
	case CALLBACK_ENUM_CHANNEL_BANS:
	{
		...
		COPY(channel);
		COPY_STR_ARRAY(bans, numBans, 821);
		break;
	}
	...
	case CALLBACK_NICK_ERROR:
	{
		...
		COPY(nick);
		COPY_STR_ARRAY(suggestedNicks, numSuggestedNicks, 830);
		break;
	}
	...
	case CALLBACK_NEW_USER_LIST:
	{
		...
		COPY(channel);
		COPY_STR_ARRAY(users, numUsers, 848);
		COPY_INT_ARRAY(modes, numUsers, 849);
		break;
	}
	...
	case CALLBACK_GET_GLOBAL_KEYS:
	{
		...
		COPY(user);
		COPY_STR_ARRAY(keys, num, 869);
		COPY_STR_ARRAY(values, num, 870);
		break;
	}
	...
	case CALLBACK_GET_CHANNEL_KEYS:
	{
		...
		COPY(channel);
		COPY(user);
		COPY_STR_ARRAY(keys, num, 880);
		COPY_STR_ARRAY(values, num, 881);
		break;
	}
