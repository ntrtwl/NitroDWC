...
)
{
	...
	assertWithLine(iconnection != NULL, 35);
	assertWithLine(result != GP_NO_ERROR, 36);
	assertWithLine((fatal == GP_FATAL) || (fatal == GP_NON_FATAL), 37);
...
)
{
	// Call the callback.
	/////////////////////
	assertWithLine(data->callback.callback != NULL, 110);
	assertWithLine(data->arg != NULL, 111);
