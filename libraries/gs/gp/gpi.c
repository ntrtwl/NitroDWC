...
)
{
	...
	*connection = NULL;
	...
	GPIConnection * iconnectionDebug = iconnection;
	iconnectionDebug->debugBuffer = NULL;
	gpiDebug((GPConnection *)&iconnection, "\x0A\x0A\x0A\x0A\x0A*************\x0AgpiInitialize\x0A", iconnectionDebug);
...
)
{
	...
	do
	{
		...
		while((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
		{
			...
			// Check for an id.
			///////////////////
			char * inputBuffer = iconnection->inputBuffer;
			str = strstr(inputBuffer, "\\id\\");
			if(str != NULL)
			{
				// Get the id.
				//////////////
			...
			else
			{
				// Check for an error.
				//////////////////////
				if(gpiCheckForError(connection, inputBuffer, GPITrue))
				{
					return GP_SERVER_ERROR;
				}
				else {
					inputBuffer = iconnection->inputBuffer;
					if(strncmp(inputBuffer, "\\bm\\", 4) == 0)
					{
						CHECK_RESULT(gpiProcessRecvBuddyMessage(connection, inputBuffer));
					}
					else if(strncmp(inputBuffer, "\\ka\\", 10) == 0)
					{
